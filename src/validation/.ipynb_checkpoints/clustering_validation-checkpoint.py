...
def get_Jaccard_Table(adata_1, adata_2, 
    ...
    Returns:
    - jaccard_df (DataFrame): Cluster-by-cluster Jaccard index matrix.
    """
...
def plot_jaccard_matrix(
    ...
    """
    Plots a heatmap of Jaccard similarity scores between clusters.

    Parameters:
    - jaccard_df (DataFrame): Matrix of Jaccard indices.
    - ax (matplotlib Axes): Axes on which to plot.
    - annot_text (bool): Whether to annotate cells with text.
    - title (str): Title of the plot.
    - cmap: Optional seaborn/matplotlib colormap.
    - x_lab, y_lab (str): Axis labels.
...
def get_top_de_genes(adata, top_n=10):
    """
    Retrieves the top N DE genes per cluster from a ranked DE result.

    Parameters:
    - adata (AnnData): Annotated dataset with DE results.
    - top_n (int): Number of top genes to retrieve per cluster.

    Returns:
    - dict of DataFrames: One per cluster, with top DE gene statistics.
    """
...
def get_zscore_corr_summary(adata_1, adata_2, clustering1, clustering2, 
    ...
    """
    Computes Pearson correlation of Z-scores for shared DE genes
    across matched clusters between two datasets/timepoints.

    Parameters:
    - adata_1, adata_2: AnnData objects from time 1 and 2.
    - clustering1, clustering2: Cluster labels at time 1 and 2.
    - cluster_label (str): Name of cluster label column in AnnData.obs.
    - n_top_genes (int): Top DE genes to consider (used only for normalization).
    - overlap_lst (list): Cluster IDs to compare.
    - plot (bool): If True, show scatter plots for each cluster.

    Returns:
    - DataFrame: Correlation stats per shared cluster.
    """
    if overlap_lst is not None:
        mask1 = np.isin(clustering1, overlap_lst)
        mask2 = np.isin(clustering2, overlap_lst)

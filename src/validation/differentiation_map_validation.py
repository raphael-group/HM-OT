...
def NPMI(Tij, gi, gj, eps=1e-12):
    """
    Compute the normalized pointwise mutual information (NPMI).

    Parameters:
    -----------
    Tij : float
        Joint probability/mass for event (i,j).
    gi : float
        Marginal probability/mass for event i.
    gj : float
        Marginal probability/mass for event j.
    eps : float
    ...
    --------
    npmi_val : float
        NPMI value, bounded in [-1,1], higher means stronger association.
    """
...
def score_from_graph(_Qs, _Ts, _labels_Q, timepoints,
    ...
    """
    Score each edge in a “ground‐truth” graph by computing NPMI
    between cluster types over successive timepoints.

    Parameters:
    -----------
    _Qs : list of np.ndarray
        Soft‐assignment matrices at each timepoint (length T).
    _Ts : list of np.ndarray
        Transport (transition) matrices between timepoints, same length as _Qs−1.
    _labels_Q : list of list[str]
        Cluster labels for each column of each Q matrix.
    timepoints : list of str
        Names or identifiers for each timepoint.
    G : nx.DiGraph
        Directed graph of known type transitions.
    labels_G : dict[node,str]
        Mapping of graph node → cell‐type label.
    edges_df : pd.DataFrame
        DataFrame with columns ["x","y"] listing node pairs (edges) to score.
    diagonal_from_G : bool
        If True, score only self‐transitions for types present in G. Otherwise
        score all self‐transitions in the cluster label sets.

    Returns:
    --------
    edge_scores : dict[(type1,type2), list[float]]
        For each annotated edge, a list of NPMI scores across each interval.
    edge_scores_diagonal : dict[type, list[float]]
        For each “self” edge, a list of NPMI scores across intervals.
    """
    edge_scores = {}
    """
...
def yield_differentiation_graph(nodes_df, edges_df, plotting=False, include_downstream=False):
    ...
    """
    Build and optionally visualize a directed differentiation graph.

    Parameters:
    -----------
    nodes_df : pd.DataFrame
        Must contain columns ["meta_group","celltype_new","celltype_num"].
    edges_df : pd.DataFrame
        Must contain columns ["x","y","edge_type"].
    plotting : bool
        If True, draw the graph with a layered layout.
    include_downstream : bool
        If True, add extra “indirect” edges for all descendants.

    Returns:
    --------
    G : nx.DiGraph
        The constructed graph with node attributes:
          - label: human‐readable cell type
          - size: numeric for plotting
    labels_G : dict[node,str]
        Mapping of node ID → label attribute.
    """
...
def compute_centroids(X, Q):
    ...
    Parameters:
    - X (ndarray): Data matrix (n_samples x n_features).
    - Q (ndarray): Soft assignment matrix (n_samples x n_clusters),
                   where Q[i, j] is the membership of sample i in cluster j.

    Returns:
    - centroids (ndarray): Matrix of cluster centroids (n_clusters x n_features).
    """
    gQ = np.sum(Q, axis = 0)
...
def evaluate_coclusters(Qs_ann, Qs_u, Ts_u, Ts_s, Ts_m,
                        X1, X2, X3):
    """
    Evaluates cluster similarity over time using cosine similarity of centroids,
    considering annotated and unsupervised clustering and different transport maps.

    Parameters:
    - Qs_ann (list): List of annotated cluster assignment matrices at t1, t2, t3.
    - Qs_u (list): List of unsupervised cluster assignment matrices at t1, t2, t3.
    - Ts_u, Ts_s, Ts_m (tuple): Transition matrices from t1->t2 and t2->t3
      for unsupervised, supervised, and moscot methods respectively.
    - X1, X2, X3 (ndarray): Expression data at timepoints t1, t2, t3.

    Output:
    - Prints weighted cosine similarity scores for each method and timepoint comparison.
    """
    Q1 = Qs_ann[0]
    Q3 = Qs_ann[2]
